...
export class ResourceGroupStep extends WebsiteCreatorStepBase {
    ...
    async prompt(): Promise<void> {
        ...
        const resourceGroupsTask = uiUtil.listAll(resourceClient.resourceGroups, resourceClient.resourceGroups.list());
...
export class AppServicePlanStep extends WebsiteCreatorStepBase {
    ...
    async prompt(): Promise<void> {
        ...
        const plansTask = uiUtil.listAll(client.appServicePlans, client.appServicePlans.list()).then(result => {
        ...
        // Cache app service plan separately per subscription
        const pickedItem = await this.showQuickPick(plansTask, quickPickOptions, `NewWebApp.AppServicePlan/${subscription.id}`);
...
export class WebsiteNameStep extends WebsiteCreatorStepBase {
    ...
    private async generateRelatedName(siteName: string): Promise<string> {
        ...
        const resourceGroupsTask = uiUtil.listAll(resourceClient.resourceGroups, resourceClient.resourceGroups.list());
        const plansTask = uiUtil.listAll(webSiteClient.appServicePlans, webSiteClient.appServicePlans.list());
